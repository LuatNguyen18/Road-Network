...
public:
	...
	virtual void printAllPaths(int, int);
	...
	bool operator==(const Directed&);
	void operator=(const Directed&);
	void operator++();
	Directed operator+(const Directed&);
	bool operator>(const Directed&);
	friend ostream& operator<<(ostream&, const Directed&);
};
