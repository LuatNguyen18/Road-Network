...
Undirected::Undirected(Edge& e1, Vertex& v1, Vertex& v2)
{
...
//Copy constructor
Undirected::Undirected(const Undirected& und)
{
...
{
	cout << "Undirected Destructor Called" << endl;
}

//Accessing member functions
int Undirected::getnbvertex()
{
	return nbvertex;
}

int Undirected::getnbedge()
{
	return nbedge;
}

//Prints weights
void Undirected::printweight()
{
...
bool Undirected::addVertices(Vertex* vArray, int size)
{
...
bool Undirected::addEdges(Edge* obj, int a)
{
	for (int i = 0; i < a; i++)
	{
...
void Undirected::searchvertexvalue(int a)
{
	bool found = false;
	int index = 0;
...
bool Undirected::searchedge(int start, int end)
{
	for (int i = 0; i < nbedge; i++)	//Loops through to search 
	{
...
void Undirected::printAllPaths(int s, int d)
{
...
void Undirected::printpaths(int u, int d, vector<bool> visited, int path[], int& path_index)
{
	
	// Mark the current node as visited and stores it in path[]
	visited[u] = 1;
	path[path_index] = u;
	...
	else // If current vertex is not destination
	{
		...
		for (int i = 0; i < edgelist.size(); i++)
		{
			...
			//Checks if edge has vertex equal this vertex
			...
			//Checks if edge has vertex
			else if(edgelist[i].getendv().getid() == u)
			{
				//Checks if it has been visited
				if (!visited[edgelist[i].getstartv().getid()])
					//Calls function recursively
					printpaths(edgelist[i].getstartv().getid(), d, visited, path, path_index);
			}
...
void Undirected::printlead(int vertex1)
{
...
//returns true if G1and G2 have the exact same vertices and edges
bool Undirected::operator==(const Undirected& und)
{
...
void Undirected::operator=(const Undirected& g2)
{
	//Copying variables
...
Undirected Undirected::operator+(const Undirected& g)
{
	Undirected und;
...
ostream& operator<<(ostream& output, const Undirected& und)
{
	//Displaying Graph
	output << "Graph: " << endl;
